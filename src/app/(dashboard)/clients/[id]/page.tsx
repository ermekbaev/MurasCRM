  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div>
        <Link
          href="/clients"
          className="flex items-center gap-1.5 text-sm text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:text-slate-300 mb-3"
        >
        ...
        <div className="space-y-4">
            ...
            <div className="space-y-2">
              {client.phone && (
                <div className="flex items-center gap-2">
                  ...
                  <a href={`tel:${client.phone}`} className="text-sm text-gray-700 dark:text-slate-300 hover:text-violet-600">
                    {client.phone}
                  </a>
                </div>
              )}
              {client.email && (
                <div className="flex items-center gap-2">
                  ...
                  <a href={`mailto:${client.email}`} className="text-sm text-gray-700 dark:text-slate-300 hover:text-violet-600">
                    {client.email}
                  </a>
                </div>
              )}
              {client.telegram && (
                <div className="flex items-center gap-2">
                  <MessageSquare size={14} className="text-gray-400 dark:text-slate-500 shrink-0" />
                  <a href={`https://t.me/${client.telegram.replace(/^@/, "")}`} target="_blank" rel="noreferrer" className="text-sm text-gray-700 dark:text-slate-300 hover:text-violet-600">
                    {client.telegram}
                  </a>
                </div>
              )}
              {client.whatsapp && (
                <div className="flex items-center gap-2">
                  ...
                  <a href={`https://wa.me/${client.whatsapp.replace(/\D/g, "")}`} target="_blank" rel="noreferrer" className="text-sm text-gray-700 dark:text-slate-300 hover:text-green-600">
                    {client.whatsapp}
                  </a>
                </div>
              )}
              {!client.phone && !client.email && !client.telegram && !client.whatsapp && (
                <p className="text-sm text-gray-400 dark:text-slate-500">Контакты не указаны</p>
              )}
            </div>
          </Card>
              ...
              <dl className="space-y-1.5">
                {client.inn && (
                  <div className="flex justify-between text-sm">
                    <dt className="text-gray-500 dark:text-slate-500">ИНН</dt>
                    <dd className="text-gray-800 dark:text-slate-200 font-medium">{client.inn}</dd>
                  </div>
                )}
                {client.kpp && (
                  <div className="flex justify-between text-sm">
                    <dt className="text-gray-500 dark:text-slate-500">КПП</dt>
                    <dd className="text-gray-800 dark:text-slate-200 font-medium">{client.kpp}</dd>
                  </div>
                )}
                {client.ogrn && (
                  <div className="flex justify-between text-sm">
                    <dt className="text-gray-500 dark:text-slate-500">ОГРН</dt>
                    <dd className="text-gray-800 dark:text-slate-200 font-medium">{client.ogrn}</dd>
                  </div>
                )}
              </dl>
            </Card>
          ...
          {(client.legalAddress || client.postalAddress) && (
            <Card padding="md">
              ...
              {client.legalAddress && (
                <div className="mb-2">
                  <p className="text-xs text-gray-500 dark:text-slate-400 mb-0.5">Юридический</p>
                  <p className="text-sm text-gray-700 dark:text-slate-300">{client.legalAddress}</p>
                </div>
              )}
              {client.postalAddress && (
                <div>
                  <p className="text-xs text-gray-500 dark:text-slate-400 mb-0.5">Почтовый</p>
                  <p className="text-sm text-gray-700 dark:text-slate-300">{client.postalAddress}</p>
                </div>
              )}
            </Card>
          )}
          ...
          {client.notes && (
            <Card padding="md">
              <h2 className="font-semibold text-gray-800 dark:text-slate-200 mb-2">Заметки</h2>
              <p className="text-sm text-gray-600 dark:text-slate-400 whitespace-pre-line">{client.notes}</p>
            </Card>
          )}
        </div>
