    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Справочник поставщиков расходных материалов</p>
        </div>
            ...
            <Card key={s.id} padding="md">
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex gap-1 shrink-0">
                  <button onClick={() => openEdit(s)} className="p-1.5 text-gray-400 hover:text-violet-600 rounded">
                    <Edit3 size={14} />
                  </button>
                  <button onClick={() => handleDelete(s.id)} className="p-1.5 text-gray-400 hover:text-red-500 rounded">
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
              ...
              <dl className="space-y-1.5">
                ...
                {s.email && (
                  <div className="flex justify-between text-sm">
                    ...
                    <dd className="text-gray-800 truncate max-w-[160px]">{s.email}</dd>
                  </div>
                )}
                {s.materials.length > 0 && (
                  <div className="pt-1">
                    <p className="text-xs text-gray-500 mb-1">Поставляемые материалы:</p>
                ...
                {s.notes && (
                  <p className="text-xs text-gray-500 pt-1 border-t border-gray-100">{s.notes}</p>
                )}
              </dl>
            </Card>
          ))}
