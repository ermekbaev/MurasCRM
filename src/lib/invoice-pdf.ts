/**
 * НЕ ИСПОЛЬЗУЕТСЯ. Прежняя вёрстка документа для PDF.
 *
 * Здесь документ собирался второй раз, отдельно от того, что видно на экране.
 * Две вёрстки разъехались: на экране была форма по образцу заказчика, а в
 * скачанном файле — старая. Теперь PDF снимается с самой страницы, см.
 * lib/pdf-capture.ts, и источник правды один.
 *
 * Модуль оставлен по просьбе: формы Muras должны сохраниться в коде. Обратно
 * подключать не нужно — вернётся ровно то расхождение, из-за которого всё
 * переделывалось.
 */
...
// Реэкспорт для прежних импортов из этого модуля.
