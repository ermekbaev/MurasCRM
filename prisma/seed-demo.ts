...
async function main() {
  ...
  await prisma.task.createMany({
    data: [
      { title: "Печать баннера фитнес-акции", orderId: orders.o2.id, assigneeId: operator.id, type: TaskType.PRINT, status: "IN_PROGRESS", priority: Priority.URGENT, dueDate: d(1), tags: ["Срочно"] },
      { title: "Резка винила для авто", orderId: orders.o3.id, type: TaskType.CUT, status: "TODO", priority: Priority.NORMAL, dueDate: d(6) },
      { title: "Печать DTF на футболки", orderId: orders.o4.id, assigneeId: operator.id, type: TaskType.PRINT, status: "IN_PROGRESS", priority: Priority.NORMAL, dueDate: d(3) },
      { title: "Ламинация наклеек", orderId: orders.o5.id, assigneeId: operator.id, type: TaskType.LAMINATION, status: "REVIEW", priority: Priority.NORMAL, dueDate: d(1) },
      { title: "Лазерная резка таблички", orderId: orders.o6.id, assigneeId: operator.id, type: TaskType.CUT, status: "DONE", priority: Priority.NORMAL, startedAt: d(-6), finishedAt: d(-5) },
      { title: "Монтаж световой вывески", orderId: orders.o9.id, assigneeId: operator.id, type: TaskType.MOUNTING, status: "DONE", priority: Priority.NORMAL, startedAt: d(-6), finishedAt: d(-5) },
      { title: "Контроль качества визиток", orderId: orders.o8.id, assigneeId: operator.id, type: TaskType.QC, status: "DONE", priority: Priority.NORMAL, finishedAt: d(-4) },
      { title: "Препресс плакатов A1", orderId: orders.o11.id, type: TaskType.FILE_PREP, status: "TODO", priority: Priority.NORMAL, dueDate: d(5) },
      { title: "Печать подарочных коробок", orderId: orders.o12.id, assigneeId: operator.id, type: TaskType.PRINT, status: "TODO", priority: Priority.NORMAL, dueDate: d(2) },
    ],
  });
